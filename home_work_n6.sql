...
go if db_id('Academy6') is not null begin drop database Academy6;

end go create database Academy6;
...
INSERT INTO Faculties (Name) VALUES ('Computer Science2');
...
INSERT INTO Groups (DepartmentId, Name, Year) VALUES (1, 'D111', 3);

INSERT INTO Groups (DepartmentId, Name, Year) VALUES (1, 'D221', 5);
...
INSERT INTO
    Lectures (Date, SubjectId, TeacherId)
...
INSERT INTO GroupsStudents (GroupId, StudentId) VALUES (2, 4);
...
GROUP BY Building
HAVING
    SUM(Departments.Financing) > 100000;
...
FROM
    ...
    Departments
WHERE
    Lectures.Id = GroupsLectures.LectureId
...
FROM
    ...
    Students
WHERE
    Groups.Id = GroupsStudents.GroupId
...
FROM
    ...
    Curators
WHERE
    GroupsCurators.CuratorId = Curators.Id
...
FROM
    ...
    Teachers
WHERE
    Subjects.Id = Lectures.SubjectId
...
HAVING COUNT(Lectures.Id) = (
        SELECT
            MAX(LecturesCount) AS MAXLectures
        FROM (
                SELECT
...
HAVING COUNT(Lectures.Id) = (
        SELECT
            MIN(LecturesCount) AS MINLectures
        FROM (
                SELECT
                    Subjects.Id AS subId,
                    COUNT(Lectures.Id) AS LecturesCount
...
SELECT
    COUNT(GroupsStudents.Id) AS StudentsCount,
    COUNT(GroupsLectures.Id) AS LecturesCount
FROM
    Groups,
    GroupsStudents,
    GroupsLectures,
